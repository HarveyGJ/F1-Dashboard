        ...
        with st.spinner("Fetching Results"):
            try:
                with col2:
                    ...
                    tel_results = process_tel(session, driver_selection)
                    lap_results = lap_time_distributions(session, driver_selection)
                    st.pyplot(tel_results)
                    st.pyplot(lap_results)
                    st.warning(
